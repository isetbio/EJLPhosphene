function [stimulusReconstruction, stimulusReconstructionOnes, params] = irReconstruct(innerRetina, varargin)
...
p.addParameter('mosaicOffset', 0, @isnumeric);
...
mosaicOffset = p.Results.mosaicOffset;
%% 
...
stimulusReconstruction = zeros(nPixX*nX +mincoord, nPixY*nY +mincoord + 2, nFrames + nFramesRF);

stimulusReconstructionOnes = zeros(nPixX*nX +mincoord, nPixY*nY +mincoord+ 2, nFrames + nFramesRF);

for cellTypeInd = [1:4]%:length(innerRetina.mosaic)
...
for xc = 1:nX
    for yc = 1:nY
        ...
        for iFrame = 1:length(spPlot)
            ...
            if length(mosaicOffset)>1
            [mosaicSubRow, mosaicSubCol] = ind2sub([size(mosaicOffset,1) size(mosaicOffset,2)],cellTypeInd);
            centerCoords = squeeze(mosaicOffset(mosaicSubRow,mosaicSubCol,:))' + innerRetina.mosaic{cellTypeInd}.cellLocation{xc,yc};            
            else
            ...
            if max(xcoords) < size(stimulusReconstruction,1) && max(ycoords) < size(stimulusReconstruction,2) 
            stimulusReconstruction(xcoords, ycoords, tcoords) = ...
                stimulusReconstruction(xcoords, ycoords, tcoords) + ...
                tuningWeight*strf(xcgoodind,ycgoodind,:);
            stimulusReconstructionOnes(xcoords, ycoords, tcoords) = ...
                stimulusReconstructionOnes(xcoords, ycoords, tcoords) + ...
                ones(size(strf(xcgoodind,ycgoodind,:)));
            end
        end%iFrame
