function primaArray = computeElectrodeOLD(primaArray, movieInput)
...
for xPos = 1:numberElectrodesX
    for yPos = 1:numberElectrodesY
        ...
        linearActivation(xPos, yPos,:) = mean(mean(electrodeStimulus,1),2);
...
visualizeStimulusAndElectrodeActivation(electrodeCoords, fullStimulus, linearActivation, primaArray.activation, primaArray.activationDS, primaArray.activationDSoff)


end

function visualizeStimulusAndElectrodeActivation(electrodeCoords, fullStimulus, linearActivation, activation, activationDS, activationDSoff)
    ...
    % Video writer
    videoOBJ = VideoWriter('electrodeActivationOLD.mp4', 'MPEG-4'); % H264 format
