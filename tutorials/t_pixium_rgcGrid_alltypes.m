...
innerRetina = irGetLinearRespElectrode(innerRetina, innerRetinaInput, innerRetinaThreshold, params);
innerRetina2 = irGetLinearRespElectrode(innerRetina2, innerRetinaInput2, innerRetinaThreshold2, params);
innerRetina3 = irGetLinearRespElectrode(innerRetina3, innerRetinaInput3, innerRetinaThreshold3, params);
innerRetina4 = irGetLinearRespElectrode(innerRetina4, innerRetinaInput4, innerRetinaThreshold4, params);
...
movieSize = size(movrecons_on_offHealthy{mosaicInd});
...
name_str = ['prosthesis_combined_gratings_' num2str(cputime*100) '.mp4'];
...
for frame1 = 27+[1:size(movieStitch,3)-1]
    subplot(221);
    imagesc(squeeze(movingBar.sceneRGB(:,:,frame1,:)));
    caxis([0 1]);
    colormap gray; 
...
%         imagesc(movrecons_on(:,:,frame1));
        ...
        imagesc(movieStitchHealthy(:,:,frame1-20));
...
%     caxis([1*paramsRec.minR 1*paramsRec.maxR]);
    ...
    imagesc(movieStitch(:,:,frame1-20-5));  
