...
electrodeArray.width = 1*30e-6; % meters
...
% On midget
offFlag = 0; nTileRows = 1; nTileCols = 1; mosaicOffset = 0;
...
%%
tic
    spikesout  = RGB2XWFormat(mosaicGet(innerRetina1.mosaic{1},'spikes'));
    spikesout2 = RGB2XWFormat(mosaicGet(innerRetina2.mosaic{1},'spikes'));
    spikesout3 = RGB2XWFormat(mosaicGet(innerRetina3.mosaic{1},'spikes'));
    spikesout4 = RGB2XWFormat(mosaicGet(innerRetina4.mosaic{1},'spikes'));
...
%     filename1 = [reconstructionRootPath '/dat/nsResponses/' saveFile '_block_' num2str(blockNum) '.mat'];
    filename1 = [reconstructionRootPath '/dat/pixium/' saveFile '_100Hz_block_' num2str(blockNum) '.mat'];
    ...
    clear innerRetinaInput innerRetinaInput2 innerRetinaInput3 innerRetinaInput4
    close;
end
toc
...
for blockSp = 1:12000-1
    spikesoutSum(:,blockSp) = sum(spikesoutsm(:,100*(blockSp-1)+1:100*blockSp),2);
end
spikesoutSum(:,12000) = zeros(size(spikesoutsm,1),1);
rfout = RGB2XWFormat(whiteNoiseSmall)*spikesoutSum';
figure; 
cistart = 100;
for ci = 1:100
    subplot(10,10,ci);
    imagesc(reshape(rfout(:,ci+cistart),[96 96]))
end
