...
electrodeArray.width = 4*15e-6; % meters
...
testmoviebig = zeros(140,140,nFrames);
for frind = 1:nFrames
    testmoviebig(:,:,frind) = imresize(testmovie.matrix(1:40,1:40,frind)',[140 140]);
    if mod(floor(frind/10),2) == 1
        testmoviebig(:,:,frind) = 255 - testmoviebig(:,:,frind);
    end
end
...
testmovieshort = double(testmoviebig(:,:,1:nFrames))./255; 
...
movingBar.absorptions = absorptions;
movingBar.sceneRGB = testmovieshort;
...
rp = metersPerPixel*vertcat(innerRetina.mosaic{mosaicInd}.cellLocation{:});
...
%                 rgcCenter = innerRetina2.mosaic{mosaicInd}.cellLocation{xind,yind}*metersPerPixel;
                rgcCenterMosaic = innerRetina2.mosaic{mosaicInd}.cellLocation{xind,yind}*metersPerPixel;
                [mosaicSubRow, mosaicSubCol] = ind2sub([nTileRows nTileCols],mosaicInd);
                rgcCenter = rgcCenterMosaic + squeeze(mosaicOffset(mosaicSubRow,mosaicSubCol,:))';
...
recons_stim_on = cell(length(innerRetina2.mosaic));
movrecons_on = cell(length(innerRetina2.mosaic));
for mosaicInd = 1:length(innerRetina.mosaic)
...
recons_stim_on{mosaicInd} = reconsFromFilt(filterMat, spikeRespOn);
movrecons_on{mosaicInd} = reshape(recons_stim_on{mosaicInd},96,96,size(recons_stim_on{mosaicInd},2));
    
deadIndicesAll = randperm(size(spikeRespOn,1));
numberDead = round(percentDead*(size(spikeRespOn,1)));
deadIndices = deadIndicesAll(1:numberDead);
spikeRespOn(deadIndices,:) = zeros(length(deadIndices),size(spikeRespOn,2));

recons_stim_on_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOn);
...
movrecons_on_dropout{mosaicInd} = reshape(recons_stim_on_dropout{mosaicInd},96,96,size(recons_stim_on{mosaicInd},2));
...
recons_stim_off = cell(length(innerRetina2.mosaic));
movrecons_off = cell(length(innerRetina2.mosaic));
...
recons_stim_off{mosaicInd} = reconsFromFilt(filterMat, spikeRespOff);
movrecons_off{mosaicInd} = reshape(recons_stim_off{mosaicInd},96,96,size(recons_stim_off{mosaicInd},2));
    
deadIndicesAll = randperm(size(spikeRespOff,1));
numberDead = round(percentDead*(size(spikeRespOff,1)));
deadIndices = deadIndicesAll(1:numberDead);
spikeRespOff(deadIndices,:) = zeros(length(deadIndices),size(spikeRespOff,2));

recons_stim_off_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOff);
movrecons_off_dropout{mosaicInd} = reshape(recons_stim_off_dropout{mosaicInd},96,96,size(recons_stim_off{mosaicInd},2));
...
%
recons_stim_on_off = cell(length(innerRetina2.mosaic));
movrecons_on_off = cell(length(innerRetina2.mosaic)); 
for mosaicInd = 1:length(innerRetina.mosaic)
...
recons_stim_on_off{mosaicInd} = reconsFromFilt(filterMat, spikeRespOnOff);


movrecons_on_off{mosaicInd} = reshape(recons_stim_on_off{mosaicInd},96,96,size(recons_stim_on_off{mosaicInd},2));


deadIndicesAll = randperm(size(spikeRespOnOff,1));
numberDead = round(percentDead*(size(spikeRespOnOff,1)));
deadIndices = deadIndicesAll(1:numberDead);
spikeRespOnOff(deadIndices,:) = zeros(length(deadIndices),size(spikeRespOnOff,2));


recons_stim_on_off_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOnOff);


movrecons_on_off_dropout{mosaicInd} = reshape(recons_stim_on_off_dropout{mosaicInd},96,96,size(recons_stim_on_off{mosaicInd},2));
...
movieSize = size(movrecons_on_off{mosaicInd});
...
%%
for mosaicInd = 2:length(innerRetina.mosaic)
...
clear y y2
y = cell(length(innerRetinaHealthy.mosaic));
for mosaicInd = 1:length(innerRetinaHealthy.mosaic)
    
cellCtr=0; dt = .01;
maxTrials = innerRetinaHealthy.mosaic{1}.numberTrials;
nCells = size(innerRetinaHealthy.mosaic{1}.responseSpikes);
%         yout = [];
y{mosaicInd} = zeros(36,10000);
for xcell = 1:nCells(1)
    for ycell = 1:nCells(2)
%         clear yind y
        ...
        for trial = 1%:maxTrials
            ...
            yind =  innerRetinaHealthy.mosaic{mosaicInd}.responseSpikes{xcell,ycell,trial,1};
...
recons_stim_onHealthy = cell(length(innerRetina2.mosaic));
movrecons_on = cell(length(innerRetina2.mosaic));
for mosaicInd = 1:length(innerRetinaHealthy.mosaic)

spikesout = (y{mosaicInd});%double(matfOff.spikesoutsm);
spikeRespOn= downSampResp(spikesout, numcells, blocklength);
% recons_stim_onHealthy = reconsFromFilt(filtMat_on.filterMat, spikeRespOn);


recons_stim_onHealthy{mosaicInd} = reconsFromFilt(filterMat, spikeRespOn);
movrecons_onHealthy{mosaicInd} = reshape(recons_stim_onHealthy{mosaicInd},96,96,size(recons_stim_onHealthy{mosaicInd},2));
...
recons_stim_onHealthy_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOn);
...
% mov = reshape(stim,96,96,size(stim,2));
movrecons_onHealthy_dropout{mosaicInd} = reshape(recons_stim_onHealthy_dropout{mosaicInd},96,96,size(recons_stim_onHealthy{mosaicInd},2));
...
recons_stim_offHealthy = cell(length(innerRetinaHealthy2.mosaic));
movrecons_offHealthy = cell(length(innerRetinaHealthy2.mosaic));

for mosaicInd = 1:length(innerRetinaHealthy2.mosaic)
    
spikesout2 = (y2{mosaicInd});%double(matfOff.spikesoutsm);
spikeRespOff = downSampResp(spikesout2, numcells2, blocklength);

recons_stim_offHealthy{mosaicInd} = reconsFromFilt(filterMat, spikeRespOff);
movrecons_offHealthy{mosaicInd} = reshape(recons_stim_offHealthy{mosaicInd},96,96,size(recons_stim_offHealthy{mosaicInd},2));
...
recons_stim_offHealthy_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOff);
movrecons_offHealthy_dropout{mosaicInd} = reshape(recons_stim_offHealthy_dropout{mosaicInd},96,96,size(recons_stim_offHealthy{mosaicInd},2));
...
%
recons_stim_on_offHealthy = cell(length(innerRetinaHealthy2.mosaic));
movrecons_on_offHealthy = cell(length(innerRetinaHealthy2.mosaic)); 
for mosaicInd = 1:length(innerRetina.mosaic)
...
recons_stim_on_offHealthy{mosaicInd} = reconsFromFilt(filterMat, spikeRespOnOff);


movrecons_on_offHealthy{mosaicInd} = reshape(recons_stim_on_offHealthy{mosaicInd},96,96,size(recons_stim_on_offHealthy{mosaicInd},2));
...
recons_stim_on_offHealthy_dropout{mosaicInd} = reconsFromFilt(filterMat, spikeRespOnOff);


movrecons_on_offHealthy_dropout{mosaicInd} = reshape(recons_stim_on_offHealthy_dropout{mosaicInd},96,96,size(recons_stim_on_offHealthy{mosaicInd},2));
% movrecons_on_off = .5*movrecons_on_off_full./mean(movrecons_on_off_full(:));
...
%         movieStitchHealthyDropout(1+(iTileRows-1)*96:96+(iTileRows-1)*96 ,1+(iTileCols-1)*96- (iTileCols-1)*(nCut-1):96+(iTileCols-1)*96 - iTileCols*(nCut-1),:) = movrecons_on_offHealthy_dropout{mosaicCtr}(:,1:end-(nCut-1),:);
        ...
        movieStitchHealthy(1+(iTileRows-1)*96:96+(iTileRows-1)*96,1+(iTileCols-1)*96  - (iTileCols-1)*(nCut-1):96+(iTileCols-1)*96 - iTileCols*(nCut-1),:) = movrecons_offHealthy{mosaicCtr}(:,1:end-(nCut-1),:);
        movieStitchHealthyDropout(1+(iTileRows-1)*96:96+(iTileRows-1)*96 ,1+(iTileCols-1)*96- (iTileCols-1)*(nCut-1):96+(iTileCols-1)*96 - iTileCols*(nCut-1),:) = movrecons_offHealthy_dropout{mosaicCtr}(:,1:end-(nCut-1),:);
...
name_str = ['stitch_ncut_' num2str(cputime*100) '.mp4'];
...
mintest = min(testmovieshort(:));
maxtest = max(testmovieshort(:));

minelec = min(electrodeArray.activation(:));
maxelec = max(electrodeArray.activation(:));

minrecon = min(movieStitchHealthyDropout(:));
maxrecon = max(movieStitchHealthyDropout(:));

minhealthy = min(movieStitchDropout(:))
maxhealthy = max(movieStitchDropout(:))
...
for frame1 = 25+[1:size(movieStitch,3)-1]
    subplot(221);
    imagesc(squeeze(movingBar.sceneRGB(:,:,frame1,:)));
    caxis([mintest maxtest]);
    colormap gray; 
