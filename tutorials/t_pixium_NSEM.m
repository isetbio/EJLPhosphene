...
numberElectrodesY = floor(retinalPatchHeight/electrodeArray.width)+0;
...
innerRetinaHealthy2 = rgcMosaicCreate(innerRetinaHealthy,'type','offParasol','model',model);
...
%         clear yind y
        ...
        for trial = 1:maxTrials
            ...
            yind =  innerRetinaHealthy.mosaic{1}.responseSpikes{xcell,ycell,trial,1};
...
cellCtr=0; dt = .01;
maxTrials = innerRetinaHealthy2.mosaic{1}.numberTrials;
nCells = size(innerRetinaHealthy2.mosaic{1}.responseSpikes);
%         yout = [];
y2 = zeros(64,10000);
for xcell = 1:nCells(1)
    for ycell = 1:nCells(2)
%         clear yind y
        ...
        for trial = 1:maxTrials
            ...
            yind2 =  innerRetinaHealthy2.mosaic{1}.responseSpikes{xcell,ycell,trial,1};
...
% mov = reshape(stim,96,96,size(stim,2));
movrecons_on_healthy = reshape(recons_stim_on,96,96,size(recons_stim_on,2));
...
% mov = reshape(stim,96,96,size(stim,2));
%     movrecons_on_off = reshape(recons_stim_on_off,96,96,size(recons_stim_on_off,2));
movrecons_on_off_full_healthy = reshape(recons_stim_on_off,96,96,size(recons_stim_on_off,2));
movrecons_on_off_healthy = .5*movrecons_on_off_full./mean(movrecons_on_off_full(:));
figure; ieMovie(movrecons_on_off_healthy);
...
name_str = ['NSEM_pros_recon_' num2str(cputime*100) '.mp4'];
% path_str = '/Users/james/Documents/MATLAB/isetbio misc/pixium_videos/meeting_may27/';
path_str = '/Users/james/Documents/MATLAB/isetbio misc/eye_and_chip/'
...
paramsRecHealthy.minR = min(movrecons_on_healthy(:));

paramsRecHealthy.maxR = max(movrecons_on_healthy(:));

paramsRec.minR = min(movrecons_on(:));

paramsRec.maxR = max(movrecons_on(:));
...
for frame1 = 1:params.nSteps%size(movingBar.sceneRGB,3)
    subplot(221);
    imagesc(squeeze(movingBar.sceneRGB(1:30,end-29:end,frame1,:)));
%     caxis([0 1]);
    colormap gray; 
    
    subplot(222);
    for xPos = 3:numberElectrodesX-2
        for yPos = 1:numberElectrodesY-6
            hold on;
...
%     imagesc((stimulusReconstruction(1:sizeScene(2),1:sizeScene(2),frame1)));
    imagesc(movrecons_on_off_healthy(:,:,frame1));
