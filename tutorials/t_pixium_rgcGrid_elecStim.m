...
frame = 20;
    for xPos = 2:2:numberElectrodesX-2
        for yPos = 2:1:numberElectrodesY
            frame = frame + 20;
            electrodeArray.activation(xPos,yPos,frame:frame+16) = 1;
...
spikeRespOn= downSampRespSetLength(spikesout, numcells);
...
spikeRespOff = downSampRespSetLength(spikesout2, numcells2);
...
name_str = ['stitch_elec_' num2str(cputime*100) '.mp4'];
...
set(gcf,'position',[301   757   945   487]);
hold on;
for frame1 = 1:280%params.nSteps%size(movingBar.sceneRGB,3)
% for frame1 = 30+[1:size(movieStitch,3)-1]
%     subplot(221);
% %     imagesc(squeeze(movingBar.sceneRGB(:,:,frame1,:)));
%     caxis([0 1]);
%     colormap gray; 
%     
    subplot(121);
    for xPos = 2:numberElectrodesX-3
        for yPos = 2:numberElectrodesY
            hold on;
...
%     caxis([0 .5*paramsRecHealthy.maxR]);
%     caxis([0 .75*max(movrecons_on(:))]);
    caxis([min(movieStitch(:)) 1*max(movieStitchDropout(:))]);
%     title('Healthy');
